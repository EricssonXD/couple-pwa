...
- ✅ P9 installed-PWA reload-loop fix (`919695b`): user reported P6 fix broken inside iOS/Android home-screen installs — every Reload click brought banner right back. Cause: `statechange === 'activated'` fires before the new SW actually controls the page in standalone contexts; reloaded page boots under OLD SW, still observes new SW as `waiting`, re-emits banner. Fix: SW message handler now calls `skipWaiting()` AND `clients.claim()` together on `SKIP_WAITING`. claim() makes new SW the controller → fires `controllerchange` → page reloads from a known-good handoff. Auto-reload-loop on every deploy still avoided because claim() is gated behind the user-gesture message, never fires on plain install/activate. statechange listener + 10s timer kept as defensive fallbacks (reloadOnce idempotent).
...
<link
	href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&family=Fraunces:opsz,wght@9..144,400;9..144,600&display=swap"
	rel="stylesheet"
/>
```
