...
- Pre-paint redirect: an inline `<script>` in `src/app.html` (CSP-hashed via SvelteKit's `mode: 'hash'` CSP) runs synchronously in `<head>` and `location.replace()`s signed-in users away from `/` and `/welcome` based on the `ds_auth` cookie. This eliminates the welcome-flash with zero fetch overhead, regardless of cache state.
