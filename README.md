...
| `/calendar`        | Shared calendar (F8). v1 single-occurrence + v2 RFC-5545 RRULE recurring events grouped by date, with a None/Daily/Weekly/Monthly/Yearly picker. v2 reminder cron (24h + 1h push) shipped via `pg_cron`.                                                                                                                                                                                                                                        |
...
| `/repair`          | Post-conflict cooldown timer → reflection prompts → joint commitment note (F16). Notifies the partner without leaking the note body to the lockscreen.                                                                                                                                                                                                                                                                                          |
| `/pet`             | Shared Tamagotchi-lite companion — hatch a species together, earn coins from mutual actions (daily reveal, quiz wins, anniversaries…), buy treats / cosmetics / furniture, equip wardrobe. Decay is capped at mood ≥ 20 ∧ hunger ≤ 80 — never lethal, never punitive copy. Realtime `pet_state` snapshot per write, broadcast-on-commit. Diagnostics ledger at `/settings/diagnostics/pet-ledger` (read-only) with a "Reconcile wallet" button. |
...
A shared Tamagotchi-lite companion at `/pet` — both partners own one pet, hatched together, evolving through `egg → baby → grown`.

- **Earn loop.** Mutual / mostly-mutual actions credit coins via `awardForEvent` on the server (daily reveal, quiz wins, anniversaries, repair completions, etc.). Solo actions earn ½ — the present partner can keep things going. `pet_ledger` is append-only and never surfaces _which_ partner earned a row (W3 — privacy invariant; `userId` is server audit only).
- **Decay is capped, never lethal.** Mood ≥ 20 ∧ hunger ≤ 80 floors are enforced by `projectDecay()` (server source of truth, mirrored to the client so bars drift live without WS traffic). The visible state line maps to four warm-decay strings (`pet_state_fine|peckish|sleepy|resting`) — no guilt phrasing, no red badges, no push notifications about pet state.
- **Realtime.** One `pet_state` snapshot broadcast per write on the couple's private channel (broadcast-on-commit, RAF-coalesced on the receiver). Decay is **never** broadcast — clients project locally from `(stored_value, updated_at, serverNow + clockOffset)`.
- **Concurrency.** Every `pet` / `pet_wallet` write goes through an optimistic `version` check with up to 3 retries (I2 — lost-update guard).
- **Diagnostics.** `/settings/diagnostics/pet-ledger` shows the paginated audit trail and exposes a "Reconcile wallet" button (`POST /api/pet/reconcile`) that compares `SUM(coins_delta)` to the materialized wallet and writes a 0-delta `adjust` row only if drift is non-zero (idempotent — clean wallets never bump version).
- **Spec source of truth:** `pet-system.md`.
